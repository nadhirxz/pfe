...
fetch(file).then(response => response.text()).then((data) => {
	...
	$('a').hover((e) => $(e.currentTarget).children('.jumbotron').addClass('bg-info'));

	$('a').mouseleave((e) => $(e.currentTarget).children('.jumbotron').removeClass('bg-info'));

	$(document).on('click', 'a[href^="#"]', (e) => {
		e.preventDefault();
		$('html, body').animate({ scrollTop: $($.attr(e.target, 'href')).offset().top }, 500);
	});
});