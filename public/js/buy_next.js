...
function trackInput() {
    if (deliverFromPartner) {
        ...
        if (nameInput.value && thingInput.value) {
            toggleButton(true);
        } else {
            toggleButton(false);
        }
...
socket.on("price", (data) => {
    setTimeout(() => {
        ...
        switch (data.status) {
            case 0:
                resultText = js_lang_text.dlvr_info_txt(distance, data.price, data.time, data.winner);
                break;
            case 1:
                resultText = js_lang_text.dlvr_info_txt(distance, data.price, false, data.winner);
                break;
            case 2:
